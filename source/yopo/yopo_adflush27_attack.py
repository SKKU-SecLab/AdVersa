#!/usr/bin/env python3
"""
YOPO Attack Script for AdFlush_27 H2O GBM Model
Adapted from the original YOPO framework to work with the specific AdFlush_27 model.

This script supports two attack modes:
1. Constrained: Prevents changes to codemelt features (if any)
...
# AdFlush_27 specific configuration
FEATURES_PATH = '../../models/features.yaml'
ADFLUSH27_MODEL_PATH = '../../models/sota_comparison/AdFlush/GBM_1_AutoML_2_20250923_204253.zip'

class AdFlush27Dataset(Dataset):
    """Custom Dataset for AdFlush_27 data"""
    def __init__(self, data_dir, feature_columns=None):
        ...
        if label_col is not None:
            ...
            self.labels = np.zeros(len(self.features), dtype=np.int64)  # Dummy labels
...
def load_adflush27_features():
    """Load feature set for AdFlush_27 model"""
    with open(yaml_path, 'r') as f:
        config = yaml.safe_load(f)
        features=config["model_AdFlush"]
    return features
...
def compute_cost_adflush27(injected_features, orig_features, feature_list, cost_type='DC'):
    """Compute cost following original YOPO cost model for AdFlush_27 with exact YOPO values"""
    ...
    # AdFlush_27 specific feature costs
    feature_costs = {
        "content_policy_type": COST_CONTENT_MID_EASY,
        "url_length": COST_CONTENT_MID_EASY,
        "brackettodot": COST_JS_EASY,
        "is_third_party": COST_CONTENT_HARD / 2,
        "keyword_raw_present": COST_CONTENT_HARD / 2,
        ...
        "ascendant_has_ad_keyword": COST_STRUCT_HARD / 2,
    ...
    for i, feature_name in enumerate(feature_list):
        if feature_name in feature_costs:
            ...
            weight = COST_JS_HARD
        elif feature_name.startswith('ng_'):
            # N-gram features - treat as JavaScript features
            weight = COST_JS_EASY
        else:
...
def uap_attack(model, features, labels, feature_list, epsilon=10, step_size=0.1, 
    ...
    for step in range(num_steps):
        ...
        cost = compute_cost_adflush27(injected_features, features, feature_list, cost_type)
...
def calculate_perturbation_cost(perturbation, feature_list, cost_type='DC'):
    ...
    yopo_cost = compute_cost_adflush27(dummy_perturbed, dummy_original, feature_list, cost_type).item()
...
def main():
    parser = argparse.ArgumentParser(description='YOPO Attack on AdFlush_27 Model')
    parser.add_argument('--data-path', type=str,
                       default='../../dataset/testing',
                       help='Directory containing target datasets')
    parser.add_argument('--h2o-model-path', type=str, default=ADFLUSH27_MODEL_PATH,
                       help='Path to H2O MOJO model file')
    ...
    parser.add_argument('--sampling-size', type=int, default=60000,
                       help='Number of samples for attack (recommended: 20k, 40k, 60k)')
    parser.add_argument('--output-dir', type=str, default='adflush27_attack_results',
    ...
    feature_list = load_adflush27_features()
    print(f"Loaded AdFlush_27 feature set: {len(feature_list)} features")
    ...
    # Load data
    dataset = AdFlush27Dataset(args.data_path, feature_list)
    ...
    for iteration in range(args.num_iterations):
        ...
        for batch_features, batch_labels in train_dataloader:
            ...
            # Save perturbation for this iteration
            mode_str = "constrained" if args.constrained else "unconstrained"
            perturbation_file = os.path.join(args.output_dir,
                                           f"perturbation_adflush27_{mode_str}_eps{args.epsilon}_{args.cost_type}_{args.sampling_size}_iter{iteration+1}.npy")
            np.save(perturbation_file, perturbation.cpu().numpy())

            # Load and evaluate against H2O model
            try:
                ...
                print(f"  H2O Model ASR: {h2o_asr:.4f}")
                ...
                print(f"  H2O Accuracy Drop: {h2o_metrics['accuracy_drop']:.4f}")
                print(f"  H2O F1 Drop: {h2o_metrics['f1_drop']:.4f}")

                # Store H2O results
                iteration_results['h2o_asr'].append(h2o_asr)
                iteration_results['h2o_accuracy_drop'].append(h2o_metrics['accuracy_drop'])
                iteration_results['h2o_f1_drop'].append(h2o_metrics['f1_drop'])
                iteration_results['h2o_precision_drop'].append(h2o_metrics['precision_drop'])
                iteration_results['h2o_recall_drop'].append(h2o_metrics['recall_drop'])
    ...
    # Add H2O statistics if available
    if iteration_results['h2o_asr']:
        statistics['h2o_model'] = {
            'asr': calc_stats(iteration_results['h2o_asr']),
            'accuracy_drop': calc_stats(iteration_results['h2o_accuracy_drop']),
            'f1_drop': calc_stats(iteration_results['h2o_f1_drop']),
            'precision_drop': calc_stats(iteration_results['h2o_precision_drop']),
            'recall_drop': calc_stats(iteration_results['h2o_recall_drop'])
        }
    ...
    if 'h2o_model' in statistics:
        print("\nH2O MODEL:")
        print(f"  ASR:          mean={statistics['h2o_model']['asr']['mean']:.4f}, "
              f"median={statistics['h2o_model']['asr']['median']:.4f}, "
              f"std={statistics['h2o_model']['asr']['std']:.4f}")
        print(f"  Accuracy Drop: mean={statistics['h2o_model']['accuracy_drop']['mean']:.4f}, "
              f"median={statistics['h2o_model']['accuracy_drop']['median']:.4f}, "
              f"std={statistics['h2o_model']['accuracy_drop']['std']:.4f}")
        print(f"  F1 Drop:      mean={statistics['h2o_model']['f1_drop']['mean']:.4f}, "
              f"median={statistics['h2o_model']['f1_drop']['median']:.4f}, "
              f"std={statistics['h2o_model']['f1_drop']['std']:.4f}")
    ...
    # Save complete statistics to JSON
    mode_str = "constrained" if args.constrained else "unconstrained"
    stats_file = os.path.join(args.output_dir,
                             f"statistics_adflush27_{mode_str}_eps{args.epsilon}_{args.cost_type}_{args.sampling_size}.json")
